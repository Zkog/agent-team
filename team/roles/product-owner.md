...
`$TEAM` means `.claude/skills/agent-team/team`. Your clone is `po/`. Your lane is `.team/backlog/` on main, plus `.team/icons/` when you order icons. Nothing else.
...
- If the story obviously needs icons that do not exist yet, write `.team/icons/NNN-slug.md` from `$TEAM/templates/icons.md` — one line per icon: name, what it depicts, where it is used. Commit `team(po): icons NNN`, push. The UX draws them in parallel; nothing waits for you. Answer anything the UX leaves under `## Questions`. Do not describe the drawing; that is their job.
...
- Edit anything outside `.team/backlog/` and `.team/icons/`.
